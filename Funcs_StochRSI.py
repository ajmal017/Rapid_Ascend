...
def profitage(Coin, short, long, signal, wait_tick=3, Date='2019-09-25', excel=0, get_fig=0):
    ...
    for i in range(len(df)):
        ...
        if df['StochRSI_K'].iloc[i] <= entry_rsi_value:
            trade_state[i] = 1

        elif df['StochRSI_K'].iloc[i] >= exit_rsi_value:
            trade_state[i] = 2
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['PLX', 'FCT', 'THETA', 'WOM', 'AOA', 'SOC', 'RNT', 'WAXP', 'FX']
    # TopCoin = ['THETA']
    # print(Date)
    # quit()
    #     # Date = '2020-02-27'
