...
def made_x(file, input_data_length, model_num, check_span, get_fig, crop_size=None, sudden_death=0):
    ...
    if len(ohlcv_data) != 0:
        ...
        ohlcv_data[:, [-6]] = min_max_scaler(ohlcv_data[:, [-6]])
        # MACD = ohlcv_data[:, [-5]]
        # MACD_Signal = ohlcv_data[:, [-2]]
        # MACD_OSC = ohlcv_data[:, [-3]]
        ...
        for i in range(crop_size, len(ohlcv_data)):  # 마지막 데이터까지 다 긇어모은다.
            ...
            x = np.concatenate(
                (scaled_pricema[:, :4], scaled_volume, scaled_pricema[:, 4:], CMO, scaled_OBV, RSI, scaled_MACD), axis=1)
