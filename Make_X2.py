...
def low_high_ma(Coin, input_data_length, ip_limit=None, trade_limit=None, crop_size=300, sudden_death=0):
    ...
    if len(ohlcv_data) != 0:
        ...
        for i in range(crop_size, len(ohlcv_data) + 1):  # 마지막 데이터까지 다 긇어모은다.
            ...
            scaled_price = min_max_scaler(group_x[:, :4])
            # scaled_volume = min_max_scaler(group_x[:, [4]])
            scaled_MA60 = min_max_scaler(group_x[:, [-2]])
            # scaled_OBV = min_max_scaler(group_x[:, [-1]])
            x = np.concatenate((scaled_price, scaled_MA60), axis=1) + sudden_death  # axis=1, 세로로 합친다
            # x = scaled_price + sudden_death  # axis=1, 세로로 합친다
            group_x = x[-input_data_length:]
            # print(group_x[-1:, 1:2])
            # print(group_x.shape)
...
def made_x_ma(file, input_data_length, model_num, check_span, get_fig, crop_size=500, sudden_death=0):
    ...
    if len(ohlcv_data) != 0:
        ...
        #       Exstracting fiexd X_data       #
        sliced_ohlcv = np.concatenate((price, MA60), axis=1)
