...
| 300_39         | IDL 300                                                      | 중간 고저점이 좀 더 나온다.                                  |
| 54_18          | 급상승 트래픽 찾기 잘하면 살릴 수 있을듯                     |                                                              |
| 54_40          | 급상승 찾기 데이터셋 구축                                    |                                                              |
...
* 애초에 IDL 을 54 로 맞춰놓으면, 추세 반영이 어렵겠지 

* 거래 로직을 다 때려박아놓는다.

* 급상승 예측 데이터에서 RESCALED 된 데이터 기준으로 라벨링
...
* 실제 거래에서는 SCALING 이 실시간으로 이루어진다는 점을 고민해본다.

  * np.max >> 일단은 수용해야할 오차범위인가.. (데이터 스케일 사이즈와는 무관하지만 y_pred_fluc_close 의 최대값)

* <u>**DATASET 에 실시간 CROP 적용은 무분별해진다. (변동폭도 동일함을 확인함)**</u>
...
>
> CROP_SIZE 안하면 밑 라인에서만 저점이 잡혀버린다.
...
> * **최근 저점을 가장 잘 예측하는 SLICE / CROP_SIZE 를 찾는다.**
> * 출력되는 Y 데이터의 갯수를 동일하게 맞춰주면 LIMIT_LINE 이 동일하게 적용되겠지
...
> 어떤 CROP_SIZE 를 기준으로 예측할 건지 : 가장 고점을 잘 예측하는 CROP_SIZE
